...
const updateExpenses = () => {
  ...
  function handleFilter() {
    const { isCatagoryFilterOn, isDateFilterOn, isSearchboxOn } =
      filterStateFLag;
    ...
    if (isCatagoryFilterOn && isDateFilterOn && isSearchboxOn) {
      const filteredData = fetchedData.filter((expense) => {
        if (
          expense.category === catagory &&
          expense.date === date &&
          (expense.date.includes(search_value) ||
            expense.category.includes(search_value) ||
            expense.amount.includes(Number(search_value)) ||
            expense.expense.includes(search_value))
        ) {
          return expense;
        }
      });
  ...
  useEffect(() => {
    // if the filtered values change paination is called
    handlePagination();
  }, [filteredData, pageNumber]);

  useEffect(() => {
    // if any data is modified on state , syncing it with ui
    setFilteredData(fetchedData);
    handleFilter();
  }, [fetchedData]);
  ...
  return (
    <div className="flex flex-col mt-12 mb-2 items-center  min-h-[80vh]">
      <h1 className="font-bold text-5xl text-center my-4 ">
        Track And Update Expenses
      </h1>
      <div
        id="update-container"
        className="sm:w-[60vw] w-[50%] mt-4 font-semibold mb-12"
      >
        <div
          id="update-options-container"
        ...
        >
          <div className="flex flex-col justify-between w-1/3 items-center">
            ...
            <input
              ...
              name="search-box"
              id="search-box"
              placeholder="Search By Name, Amount ,Catagory Or Date"
          ...
          <div className="flex flex-col items-center gap-1 justify-between ">
            <label htmlFor="catagory-search">Filter By Catagory</label>
            <select
              ...
              id="catagory-search"
              className="outline-[1.35px] w-full outline-indigo-900 px-2 py-1 rounded "
            >
        ...
        <div className="flex justify-between ">
          <span>
            Showing {numberOfPages < 1 ? 0 : pageNumber} of {numberOfPages}{" "}
            entries
          </span>
          <div>
            <PaginationButton
